...
do { \
	...
	if (cond) \
		regnum = processCodeList_int(it->trueCodes, regnum, runPatch); \
	else \
		regnum = processCodeList_int(it->falseCodes, regnum, runPatch); \
} while (0)
 ...
 */
int CodeParser::processCodeList_int(const list<Code> &codes, int regnum, bool runPatch)
{
	for (list<Code>::const_iterator it = codes.begin(); it != codes.end(); ++it)
	{
		if (!runPatch && it->isPatch)
			continue;
 ...
 */
void CodeParser::processCodeList(bool runPatch)
{
	processCodeList_int(m_codes, -1, runPatch);
}
...
int CodeParser::readCodes(istream &stream)
{
	...
	static const char codemagic[7] = {'c', 'o', 'd', 'e', 'v', '5', '1'};
